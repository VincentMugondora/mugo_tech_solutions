...
const ServicePageTemplate = ({
  ...
  <div className="bg-white min-h-screen">
    {/* Hero Section */}
    <section className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-8 py-12 px-4">
      ...
      <div className="flex-1 w-full">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-blue-900 mb-2">{title}</h1>
        <p className="text-gray-700 text-lg mb-4">{intro}</p>
        {cta && <div className="mb-4">{cta}</div>}
      </div>
      {/* Right: Image */}
      <div className="flex-1 w-full flex items-center justify-center">
        <img src={image} alt={title} className="w-80 h-80 object-cover rounded-full shadow-lg border-8 border-orange-100" />
      </div>
    </section>
